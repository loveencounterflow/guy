(function() {
  'use strict';
  var GUY_props, H, builtins, def, def_oneoff, hide, no_such_value, props;
  ...
  H.types.declare('guy_props_tree_cfg', {
    tests: {
      ...
      "@isa.boolean x.leaves": function(x) {
        return this.isa.boolean(x.leaves);
      },
      "@isa.boolean x.branches": function(x) {
        return this.isa.boolean(x.branches);
      }
    }
  });
  ...
  this._walk_keys = function*(owner, cfg) {
    var key, ref, seen, y;
    seen = new Set();
    ref = this._walk_keyowners(owner, cfg, 0);
    for (y of ref) {
      ({key} = y);
      if (seen.has(key)) {
        continue;
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._walk_keyowners = function*(owner, cfg, depth) {
    var d, error, i, key, len, proto_owner, ref;
    if ((cfg.depth != null) && depth > cfg.depth) {
      // urge '^3354^', owner
      return null;
    }
    ...
    if ((proto_owner = Object.getPrototypeOf(owner)) != null) {
      yield* this._walk_keyowners(proto_owner, cfg, depth + 1);
    }
    return null;
  };
  ...
  // TREE
  //-----------------------------------------------------------------------------------------------------------
  this.tree_leaves = function(owner, cfg) {
    return H.types.validate.guy_props_tree_cfg((cfg = {...H.types.defaults.guy_props_tree_cfg, ...cfg}));
  };

  //-----------------------------------------------------------------------------------------------------------
  this._walk_tree = function*(owner, cfg) {
    var key, ref, ref1, subkey;
    ref = this._walk_keys(owner, {
      allow_any: true
    });
    for (key of ref) {
      if ((this./* TAINT use appropriate cfg */keys(owner[key], {
        depth: 0
      })).length > 0/* TAINT should not actually retrieve key */) {

      } else {

      }
      yield [key];
      ref1 = this._walk_tree(owner[key], cfg);
      for (subkey of ref1) {
