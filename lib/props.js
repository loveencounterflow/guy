(function() {
  'use strict';
  var H, builtins, def, def_oneoff, hide, misfit, no_such_value, props;
      ...
      R = new Proxy(cfg.target, {
        ...
        get: (target, key) => {
          ...
          if ((value = props.get(target, key, no_such_value)) === no_such_value) {
            throw new Error(`^guy.props.Strict_owner@1^ ${this.constructor.name} instance does not have property ${H.rpr(key)}`);
          }
          return value;
        }
      });
  ...
  this._walk_tree = function*(owner, cfg) {
    ...
    for (key of ref) {
      yield [/* TAINT use appropriate cfg */ key];
      ref1 = this._walk_tree(owner[key], {
        allow_any: true
      });
      for (subkey of ref1) {
        yield [/* TAINT use appropriate cfg */ key, subkey].flat();
      }
    }
    return null;
  };
