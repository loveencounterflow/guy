(function() {
  ...
  this._walk_lines_with_positions = function*(path, chunk_size) {
    var FS, buffer, byte_count, byte_idx, fd, flush, is_cr, is_lf, line, line_cache, lnr, nl_cache, prv_was_cr;
    FS = require('node:fs');
    fd = FS.openSync(path);
    byte_idx = 0;
    prv_was_cr = false;
    ...
    flush = function*() {
      ...
      xxxxxxxxx = function*() {
        ...
        if (prv_was_cr && is_lf) {
          ...
          return console.log('^32-2^', 'is_cr', '---------->', is_cr, is_lf, nl_cache);
        } else if (is_lf) {
          line_cache.push(buffer.subarray(0, next_idx));
          ...
          line_cache.push(buffer.subarray(0, next_idx));
          return console.log('^32-4^', 'no nl', '---------->', is_cr, is_lf, nl_cache);
        }
      };
      return null;
    };
  ...
  this._walk_lines_get_next_line_part = function(buffer, first_idx, may_have_cr = true, may_have_lf = true) {
