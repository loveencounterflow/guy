(function() {
  'use strict';
  var C_cr, C_cr_buffer, C_empty_buffer, C_empty_string, C_lf, C_lf_buffer, H, defaults, misfit, platform, rpr;
  ...
  this.walk_lines_with_positions = function*(path, cfg) {
    var chunk_size, count, d, encoding, ref, trim;
  ...
  this._walk_lines_with_positions = function*(path, chunk_size = 16 * 1024) {
    var buffer, eol, eol_cache, flush, has_extra_cr, line_cache, lnr, material, ref, ref1, y;
    line_cache = [];
    eol_cache = [];
    lnr = 0;
    //.........................................................................................................
    ...
    ref = this.walk_buffers(path, {chunk_size});
    //.........................................................................................................
    for (buffer of ref) {
      ref1 = this._walk_lines__walk_advancements(buffer);
      for (y of ref1) {
        ({material, eol} = y);
        // console.log '^3353^', material, eol, ( eol is C_cr_buffer ), ( eol is C_lf_buffer )
        if ((eol_cache.length > 0) && !(((eol_cache.at(-1)) === C_cr_buffer) && eol === C_lf_buffer)) {
          yield* flush();
        }
        switch (eol) {
          case C_cr_buffer:
            if ((eol_cache.length === 1) && (eol_cache[0] === C_cr_buffer)) {
              console.log('^3353^', 'C_cr_buffer', line_cache, material, eol_cache.length === 1, eol_cache[0] === C_cr_buffer);
              yield* flush();
            }
