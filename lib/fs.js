(function() {
  'use strict';
  var H, defaults, misfit, platform;
  ...
  this.walk_lines = function*(path, cfg) {
    var chunk_size, count, encoding, line, newline, ref;
    H.types.validate.guy_fs_walk_lines_cfg((cfg = {...defaults.guy_fs_walk_lines_cfg, ...cfg}));
    H.types.validate.nonempty_text(path);
    ({chunk_size, newline, encoding} = cfg);
    //.........................................................................................................
    count = 0;
    ref = this._walk_lines(path, chunk_size);
    for (line of ref) {
      count++;
      yield (encoding != null ? line.toString(encoding) : line);
    }
    if (count === 0) {
      yield (encoding != null ? '' : Buffer.from(''));
    }
    return null;
  };

  //-----------------------------------------------------------------------------------------------------------
  this._walk_lines = function*(path, chunk_size) {
    var FS, buffer, byte_count, byte_idx, cache, cr, fd, flush, is_cr, is_lf, lf, prv_was_cr, walk_lines;
    FS = require('node:fs');
    fd = FS.openSync(path);
    cache = [];
    byte_idx = 0;
    ...
    walk_lines = function*(buffer) {
      ...
      while (true) {
        next_idx_cr = (next_idx_cr = buffer.indexOf(cr)) === -1 ? 2e308 : next_idx_cr;
        next_idx_lf = (next_idx_lf = buffer.indexOf(lf)) === -1 ? 2e308 : next_idx_lf;
        prv_was_cr = is_cr;
        is_lf = false;
        is_cr = false;
        //.....................................................................................................
        if (next_idx_cr === 2e308) {
          if (next_idx_lf === 2e308) {
            cache.push(buffer);
            break;
          }
          ...
          if ((next_idx_lf === 2e308) || (next_idx_cr < next_idx_lf)) {
