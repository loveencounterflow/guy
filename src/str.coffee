...
@get_loggers = ( badge = null ) ->
  ...
  R       =
    ...
    plain:    ( P... ) => @log ( @grey get_timestamp() ) + ( @grey ' ▶ '        ) + prefix, P...
...
@debug                    = @get_writer process.stdout, C.cyan, C.reset + '\n'


console.log @rpr @pen [ 'foo', 42, 'bar', true, ]
@echo 'helo'
@debug @rpr @pen [ 'foo', 42, 'bar', true, ]
{ alert, debug, help, info, plain, praise, urge, warn, whisper, } = @get_loggers 'GUY'
alert   "alert    XXXXXXX"
debug   "debug    XXXXXXX"
help    "help     XXXXXXX"
info    "info     XXXXXXX"
